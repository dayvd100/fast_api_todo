...
from schemas.schemas import Message, Token, UserList, UserPublic, UserSchema
    ...
    if db_user:
        if db_user.username == user.username:
            raise HTTPException(
                ...
                detail=f'Username:{user.username} is already being used',
            )

        elif db_user.email == user.email:
            raise HTTPException(
                status_code=HTTPStatus.BAD_REQUEST,
                detail=f'Email:{user.email} is already being used',
            )
    db_user = User(
